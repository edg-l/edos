...
use elibc::{
    graphics::{Color, Screen, TextStyle, FontWeight, RasterHeight},
    println,
};
...
pub extern "C" fn main() -> i32 {
    ...
    // Create text styles
    let white_style = TextStyle::new(Color::WHITE)
    ...
    screen.draw_text(50, 50, "Hello, World!", &white_style).unwrap();
    screen.draw_text(50, 100, "edos Kernel text Demo", &red_style).unwrap();
    screen.draw_text(50, 160, "Text with background", &green_style).unwrap();
    ...
    screen.draw_text_wrapped(50, 280, long_text, &wrap_style, 600).unwrap();

    // Multi-line text demonstration
    let multiline_text = "Line 1\nLine 2\nrust > C\nchiller is a botter";
    screen.draw_text(50, 600, multiline_text, &white_style).unwrap();
