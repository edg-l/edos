...
impl DirtyRegion {
    ...
    #[allow(
        ...
        reason = "every `n -= 1` is followed at once by `break 'pairs`, so the \
                  ranges the loops already captured are never read again; the \
                  outer `loop` re-enters with the new `n`"
    )]
