//! The EFS checker, shared verbatim between the host `efs-fsck` binary and the
//! in-EDOS `/bin/fsck`, so the two cannot reach different verdicts on the same
//! image.
...
/// Check `args.image`, print the report, and exit with the fsck exit code.
