...
impl Procfs {
    ...
    fn render_journal_stats() -> String {
        ...
        let (active, sealed, pending, tracked) = BlockPageCache::global()
            .all_journals()
            .iter()
            .fold((0usize, 0usize, 0usize, 0usize), |(a, s, p, tr), j| {
                let (ja, js, jp, jt) = j.depths();
                (a + ja, s + js, p + jp, tr + jt)
            });
        let mut out = format!(
            concat!(
                "commits: {}\nempty_commits: {}\ncheckpoints: {}\n",
    ...
    /// The display's interrupts and the pointer's path to its plane.
    ///
    /// `cursor_max_inflight` above 1 means cursor commands overlap, so a
    /// command's buffer can still be unread when the next is written;
    ...
    fn render_gpu_stats() -> String {
        ...
        alloc::format!(
            ...
            CURSOR_REDUNDANT.load(Ordering::Relaxed),
        )
    }
