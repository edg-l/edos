...
macro_rules! impl_linked {
    ($type:ty, $field:ident) => {
        unsafe impl $crate::util::intrusive_list::Linked for $type {
            #[inline]
            unsafe fn links(ptr: *const Self) -> *mut $crate::util::intrusive_list::Link {
                unsafe { ::core::ptr::addr_of_mut!((*ptr.cast_mut()).$field) }
            }

            #[inline]
            unsafe fn from_link(
                link: *mut $crate::util::intrusive_list::Link,
            ) -> *mut Self {
                unsafe {
                    (link as *mut u8)
...
mod tests {
    ...
    /// Test container with a single link.
    struct Node {
        value: u64,
        link: Link,
    }
    ...
    // Manual Linked impl (the macro uses crate paths that don't resolve in
    // unit-test context).
    ...
    fn drain_partial_drop_unlinks_remaining() {
        ...
        // Take only the first element, then drop the Drain.
        {
    ...
    // -- struct with multiple links ------------------------------------------

    /// Demonstrates that a single struct can participate in two lists by
    /// having two Link fields with separate Linked impls via wrapper types.
    struct DualNode {
        value: u64,
        link_a: Link,
        link_b: Link,
    ...
    // For multiple lists per struct, use newtype wrappers with separate
    // Linked impls. Here we just verify the two Link fields are independent.

    unsafe impl Linked for DualNode {
        unsafe fn links(ptr: *const Self) -> *mut Link {
            unsafe { core::ptr::addr_of_mut!((*ptr.cast_mut()).link_a) }
        }
        unsafe fn from_link(link: *mut Link) -> *mut Self {
            unsafe {
                (link as *mut u8)
                    .sub(core::mem::offset_of!(DualNode, link_a))
                    .cast()
            }
        }
    }
    ...
    fn dual_link_independence() {
        ...
        // link_a is linked, link_b is free.
