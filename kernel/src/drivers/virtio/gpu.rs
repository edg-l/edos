...
impl VirtioGpu {
    ...
    #[expect(
        unused,
        reason = "re-reading display info after the mode is set has no caller"
    )]
    ...
    #[expect(
        unused,
        reason = "the transport accessor, kept for the same reason the other drivers keep one"
    )]
