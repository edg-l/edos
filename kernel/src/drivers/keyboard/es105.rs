//! Spanish 105-key (ISO) keyboard layout.

use pc_keyboard::{DecodedKey, HandleControl, KeyCode, KeyboardLayout, Modifiers};

/// A standard Spanish 105-key keyboard (ISO layout, QWERTY).
///
/// Has a 2-row high Enter key with Oem5 next to left shift (ISO format).
/// AltGr provides access to @, #, |, \, ~, [, ], {, }, etc.
pub struct Es105Key;
...
impl KeyboardLayout for Es105Key {
    fn map_keycode(
        ...
        match keycode {
            ...
            // Row 1: number row
            KeyCode::Oem8 => {
                // Key left of 1: masculine ordinal / backslash
                if modifiers.is_altgr() {
                    DecodedKey::Unicode('\\')
                } else if modifiers.is_shifted() {
                    DecodedKey::Unicode('\u{00AA}') // feminine ordinal
                } else {
                    DecodedKey::Unicode('\u{00BA}') // masculine ordinal
                }
            }
            KeyCode::Key1 => {
                if modifiers.is_altgr() {
            ...
            KeyCode::Key3 => {
                if modifiers.is_altgr() {
                    ...
                    DecodedKey::Unicode('\u{00B7}') // middle dot
                } else {
            ...
            KeyCode::Key6 => {
                if modifiers.is_altgr() {
                    DecodedKey::Unicode('\u{00AC}') // not sign
                } else if modifiers.is_shifted() {
                    DecodedKey::Unicode('&')
                } else {
            ...
            KeyCode::OemMinus => {
                // ' key on Spanish layout (apostrophe / question mark)
                if modifiers.is_shifted() {
                    DecodedKey::Unicode('?')
                } else {
                    DecodedKey::Unicode('\'')
            ...
            KeyCode::OemPlus => {
                ...
                if modifiers.is_shifted() {
                    DecodedKey::Unicode('\u{00BF}') // inverted question mark
                } else {
                    DecodedKey::Unicode('\u{00A1}') // inverted exclamation mark
                }
            }
            ...
            KeyCode::E => {
                if modifiers.is_altgr() {
                    DecodedKey::Unicode('\u{20AC}') // Euro sign
                } else if map_to_unicode && modifiers.is_ctrl() {
            ...
            KeyCode::Oem4 => {
                // ` / ^ (dead key area, but we just emit the character)
                if modifiers.is_altgr() {
                    DecodedKey::Unicode('[')
                } else if modifiers.is_shifted() {
                    DecodedKey::Unicode('^')
            ...
            KeyCode::Oem1 => {
                ...
                if modifiers.is_caps() {
                    DecodedKey::Unicode('\u{00D1}') // N-tilde
                } else {
                    DecodedKey::Unicode('\u{00F1}') // n-tilde
                }
            }
            KeyCode::Oem3 => {
                // acute accent / diaeresis (dead keys, emit directly)
                if modifiers.is_altgr() {
                    DecodedKey::Unicode('{')
                } else if modifiers.is_shifted() {
                    DecodedKey::Unicode('\u{00A8}') // diaeresis
                } else {
                    DecodedKey::Unicode('\u{00B4}') // acute accent
                }
            }
            KeyCode::Oem7 => {
                // c-cedilla / } (key next to Enter on ISO, between Oem3 and Enter)
                if modifiers.is_altgr() {
                    DecodedKey::Unicode('}')
                } else if modifiers.is_caps() {
                    DecodedKey::Unicode('\u{00C7}') // C-cedilla
                } else {
                    DecodedKey::Unicode('\u{00E7}') // c-cedilla
                }
            }
            KeyCode::Return => DecodedKey::Unicode(10.into()),
            // Row 4: bottom row
            KeyCode::Oem5 => {
                // < / > (key next to left shift on ISO keyboards)
                if modifiers.is_altgr() {
                    DecodedKey::Unicode('\\')
                } else if modifiers.is_shifted() {
                    DecodedKey::Unicode('>')
