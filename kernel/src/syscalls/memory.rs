...
pub fn sys_mmap(addr: u64, length: u64, prot: u32, flags: u32) -> u64 {
    ...
    let map_addr = if addr == 0 {
        ...
        find_free_virtual_address(thread, length)
    } else {
        VirtAddr::new(addr)
    };
...
pub fn sys_munmap(addr: u64, length: u64) -> i32 {
    ...
    if let Some(mapping) = thread.memory_mappings.remove(&map_addr) {
        if mapping.size == length {
            // Unmap the memory
            if thread.memory_manager.unmap_memory(map_addr, length).is_ok() {
                0 // Success
            } else {
...
fn find_free_virtual_address(thread: &mut UserThread, length: u64) -> VirtAddr {
