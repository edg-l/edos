...
|  88 | `EfsDriver.alloc_mutex` | `BlockingMutex<()>` | `fs/efs/mod.rs:82` | Serializes bitmap allocation (`alloc_inode`/`alloc_block`) across CPUs so `mutable` (160) can be released across BPC I/O (110) without two allocators picking the same bit. Inner: frame_alloc (90, via BPC), BPC shards (110), journal locks (120–150), `mutable` (160). Placed below frame_alloc because BPC pre-allocates a frame before taking its shard lock. |
...
| 160 | `EfsDriver.mutable` | `BlockingMutex<EfsMutableState>` | `fs/efs/mod.rs:85` | Acquired by EFS callbacks under `inode.lock` (30) and (for alloc paths) `alloc_mutex` (105). True leaf when held: every site releases `mutable` before calling block_page_cache (110) or journal (120/130/150). Never nested with lower-ranked locks. |
