...
| 160 | `EfsDriver.mutable` | `BlockingMutex<EfsMutableState>` | `fs/efs/mod.rs:85` | Acquired by EFS callbacks under `inode.lock` (30) and (for alloc paths) `alloc_mutex` (32). True leaf when held: every site releases `mutable` before calling block_page_cache (110) or journal (120/130/150). Never nested with lower-ranked locks. |
...
| 180 | `AhciPort.ncq_waiters[i]` | `spin::Mutex<Option<Arc<AhciNcqOp>>>` | `drivers/ahci/port.rs` | Brief per-NCQ-slot. Taken by submit, IRQ dispatcher, TFES `fail_all_ncq_slots`, and the watchdog kthread. Holders only do `Arc::clone` / `take`; never park, never call into the allocator, never acquire inner locks. Same rank as `slot_waiters`; never co-held with another rank-180 mutex. |
