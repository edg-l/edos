...
| 105 | `EfsDriver.alloc_mutex` | `BlockingMutex<()>` | `fs/efs/mod.rs:82` | Serializes bitmap allocation (`alloc_inode`/`alloc_block`) across CPUs so `mutable` (160) can be released across BPC I/O (110) without two allocators picking the same bit. Inner: BPC shards (110), per-process mm is not reachable from here, `mutable` (160). |
